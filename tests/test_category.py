...
class TestCategoryInit:
    ...
    def test_category_init_with_products(self):
        """Создание с продуктами"""
        products = [Product("Тест", "Описание", 100.0, 10)]
...
class TestCategoryAddProduct:
    ...
    def test_add_product_increments_count(self, empty_category):
        """Увеличение счётчика"""
        product = Product("Тест", "Описание", 100.0, 10)
        empty_category.add_product(product)
        assert Category.products_count >= 1

    def test_add_string_raises_typeerror(self, empty_category):
        """Добавление строки вызывает TypeError"""
        with pytest.raises(TypeError):
            empty_category.add_product("Not a product")

    def test_add_int_raises_typeerror(self, empty_category):
        """Добавление числа вызывает TypeError"""
        with pytest.raises(TypeError):
            empty_category.add_product(123)

    def test_add_none_raises_typeerror(self, empty_category):
        """Добавление None вызывает TypeError"""
        with pytest.raises(TypeError):
...
class TestCategoryIteration:
    ...
    def test_category_iteration(self, smartphones_category):
        """Итерация по категории"""
        products = list(smartphones_category)
        assert len(products) == 3

    def test_category_iteration_values(self, smartphones_category):
...
class TestCategoryProductCount:
    ...
    def test_product_count_increases(self, empty_category):
        """Увеличение product_count"""
        initial_count = Category.products_count
        empty_category.add_product(Product("Тест", "Описание", 100.0, 10))
